...
const Capacity = () => {
    ...
    useEffect(() => {
        const fetchCapacities = async () => {
            try {
                const response = await axios.get('{process.env.BACKEND_URL}getAllCapacities');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (formData.editingCapacityId) {
                await axios.put(`{process.env.BACKEND_URL}updateCapacities/${formData.editingCapacityId}`, formData);
                setCapacities(capacities.map(cap => (cap.id === formData.editingCapacityId ? formData : cap)));
                alert('Capacidad actualizada con éxito');
            } else {
                const response = await axios.post('{process.env.BACKEND_URL}createCapacities', formData);
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`{process.env.BACKEND_URL}deleteCapacities/${id}`);
