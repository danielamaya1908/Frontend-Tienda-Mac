...
const Brands = () => {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        const response = await axios.get('https://backend-tienda-mac-production.up.railway.app/products/category/Accesorios%20de%20video/getAllBrands');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isUpdateMode) {
        await axios.put(`https://backend-tienda-mac-production.up.railway.app/products/category/Accesorios%20de%20video/updateBrand/${formData.id}`, formData);
        setBrands(brands.map(brand => (brand.id === formData.id ? formData : brand)));
        alert('Marca actualizada con éxito');
      } else {
        const response = await axios.post('https://backend-tienda-mac-production.up.railway.app/products/category/Accesorios%20de%20video/createBrand', formData);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://backend-tienda-mac-production.up.railway.app/products/category/Accesorios%20de%20video/deleteBrand/${id}`);
