...
const createPurchase = async (req, res) => {
  try {
    const { amount, currency, description, payment_method, customer_name, customer_email, reference, charge_id, userId, productId } = req.body;

    // Validar campos requeridos
    if (!amount || !currency || !description || !payment_method || !customer_name || !customer_email || !userId || !productId) {
      return res.status(400).json({ error: 'All fields are required' });
    }

    // Crear la compra
    const purchase = await Purchase.create({ amount, currency, description, payment_method, customer_name, customer_email, reference, charge_id, userId, productId });
...
const updatePurchase = async (req, res) => {
  try {
    const { id } = req.params;
    const { amount, currency, description, payment_method, customer_name, customer_email, reference, charge_id } = req.body;
